...
Both models capture \textbf{long-term growth explicitly}---why classical time-series beats tree/neural methods that cap at training range. The reliability evidence, validation protocol, data setting, and reproducibility controls are reported in the Appendix.
...
where $n = 1,096$ because Revenue and COGS are concatenated for evaluation.

\subsubsection{Theta Method}

The Theta method decomposes $y_t$ into two theta lines:
...
\textbf{Reproducibility}: All code, hyperparameters, and results documented in \texttt{scripts/forecast.py}.
