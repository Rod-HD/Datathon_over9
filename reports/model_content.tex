...
Both selected models include explicit trend or decomposition components, making them suitable for extrapolating the 2023--2024 forecast horizon. This is consistent with forecasting principles that emphasize trend and seasonal structure in time-series modeling \cite{Hyndman2021}. The ensemble choice is also consistent with forecasting competition evidence, where combinations and hybrid methods often perform strongly \cite{Makridakis2020}. The reliability evidence, validation protocol, data setting, and reproducibility controls are reported in the Appendix.
...
The Theta method is used as a decomposition-based forecasting model \cite{Assimakopoulos2000}. In this implementation, the log transformation and day-of-week factors are added as dataset-specific preprocessing and calibration choices, while ARIMA focuses on residual patterns over a 3-year window to capture short-term autocorrelation.
...
Expanding-window validation was selected to preserve temporal ordering and ensure that each validation year is forecast using only earlier data. This follows the broader time-series evaluation principle that validation should account for temporal dependence and avoid future-data leakage \cite{Bergmeir2012}.
...
The Theta method decomposes the time series into theta lines to capture long-term and short-term movements, and it is widely recognized from its strong performance in the M3 forecasting competition \cite{Assimakopoulos2000}:
